              ...
              <tbody className="divide-y divide-slate-200">
                {meet.results.map((result: any, index: number) => (
                  <tr key={index} className="table-row-hover">
                    ...
                    <td className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">
                      <div className="font-mono font-bold text-slate-600 text-sm sm:text-lg">{result.time}</div>
                    </td>
                    <td className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">
                      <div className="text-xs sm:text-sm text-slate-600 font-medium">
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-bold ${
                          result.place === 1 ? 'bg-yellow-100 text-yellow-800' :
                          result.place === 2 ? 'bg-gray-100 text-gray-800' :
                          result.place === 3 ? 'bg-orange-100 text-orange-800' :
