...
const HomePage = ({ analysisData, userSession, onClearSession, setAnalysisData }: HomePageProps) => {
  const [activeView, setActiveView] = useState<'input' | 'dashboard' | null>(null)
  const [usmsLink, setUsmsLink] = useState('')
  const [isAnalyzing, setIsAnalyzing] = useState(false)
  const [error, setError] = useState('')

  const views = [
    { id: 'input', label: 'Home', icon: '🏠', description: 'Analyze new results' },
    { id: 'dashboard', label: 'Dashboard', icon: '📊', description: 'View analysis' }
  ]
  ...
  }, [usmsLink, API_BASE_URL, setAnalysisData])

  // Determine initial view based on data availability
  const initialView = useMemo(() => analysisData ? 'dashboard' : 'input', [analysisData])
  
  // Use initialView only if activeView hasn't been set yet
  const currentView = useMemo(() => activeView || initialView, [activeView, initialView])
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex bg-slate-100/70 rounded-xl p-1 shadow-inner">
              {views.map(view => (
                <button
                  key={view.id}
                  onClick={() => setActiveView(view.id as 'input' | 'dashboard')}
                  disabled={view.id === 'dashboard' && !analysisData}
                  className={`relative px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg font-medium transition-all duration-300 text-sm sm:text-base group ${
                    currentView === view.id
                      ...
                      : view.id === 'dashboard' && !analysisData
                ...
                >
                  <div className="flex items-center space-x-1.5 sm:space-x-2">
                    <span className={`text-sm sm:text-base transition-all duration-300 ${
                      currentView === view.id ? 'scale-110' : 'group-hover:scale-105'
                    }`}>
                      {view.icon}
                    </span>
      ...
      <div className="container mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        {currentView === 'dashboard' && !analysisData && (
          <div className="text-center py-8 sm:py-12">
            <div className="bg-white/90 backdrop-blur-xl border border-white/50 rounded-3xl shadow-xl p-6 sm:p-8 max-w-md mx-auto">
              <div className="text-4xl sm:text-5xl mb-4 opacity-50">📊</div>
              <h3 className="text-lg sm:text-xl font-bold text-slate-800 mb-2">No Data Available</h3>
              <p className="text-sm sm:text-base text-slate-600 mb-4">Please analyze a USMS link first to view your dashboard.</p>
              <button
