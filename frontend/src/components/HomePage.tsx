...
const HomePage = ({ analysisData, userSession, onClearSession, setAnalysisData }: HomePageProps) => {
  const [activeView, setActiveView] = useState<'input' | 'dashboard'>('input')
  ...
  // Show input form if no data exists, otherwise show dashboard
  const defaultView = analysisData ? 'dashboard' : 'input'
                ...
                >
                  <div className="flex items-center space-x-1.5 sm:space-x-2">
                    <span className={`text-sm sm:text-base transition-all duration-300 ${
                      activeView === view.id ? 'scale-110' : 'group-hover:scale-105'
                    }`}>
                      {view.icon}
                    </span>
